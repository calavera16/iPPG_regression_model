...
# --- NEW: Import Pennylane ---
import pennylane as qml

# --- 1. Configuration ---
MANIFEST_FILE = (
    r"C:\Users\yeonj\Desktop\iPPG_new_project\manifest_preprocessed_copy.csv"
)
RESULTS_DIR = r"C:\Users\yeonj\Desktop\iPPG_new_project\results_model2_2_thinner_qnn"
NUM_EPOCHS = 50
BATCH_SIZE = 16
LEARNING_RATE = 1e-4
...
# --- NEW: Quantum Configuration ---
N_QUBITS = 4  # Number of qubits in the quantum circuit
N_QLAYERS = 2  # Number of layers in the variational circuit
# Use the fast lightning simulator if available
QUANTUM_DEVICE = "lightning.qubit"


# --- 2. Custom Dataset (No changes needed) ---
class PreprocessedVideoBPDataset(Dataset):
...
def quantum_circuit(inputs, weights):
    # Encode classical features into the quantum circuit
    qml.AngleEmbedding(inputs, wires=range(N_QUBITS))
    # Apply trainable quantum layers
    qml.StronglyEntanglingLayers(weights, wires=range(N_QUBITS))
    # Measure the expectation value of PauliZ on the first two qubits
...
class VideoBPNet(nn.Module):
    ...
    def forward(self, x):
        ...
        quantum_output = self.qnn(aggregated_features)
        out = self.final_scaler(quantum_output)
        return out


# --- 5. Plotting and History Functions ---
    ...
    with torch.no_grad():
        ...
        for subject_id, indices in subject_pbar:
            ...
            for videos, labels in subject_loader:
                ...
                with torch.cuda.amp.autocast():
                    outputs = model(videos)
                    subject_mse += mse_criterion(outputs, labels).item() * videos.size(
                        0
                    )
                    subject_mae += mae_criterion(outputs, labels).item() * videos.size(
                        0
                    )
...
# --- 7. Main Execution with K-Fold Cross-Validation ---
if __name__ == "__main__":
    ...
    full_dataset = PreprocessedVideoBPDataset(manifest_df=manifest_df)
    fold_results = []
    ...
    for fold, (train_subject_indices, test_subject_indices) in enumerate(
        ...
        # Pass the correct test_loader to the evaluation function
        detailed_metrics = evaluate_and_save_predictions(
            model, test_loader, writer, fold + 1
        )

        # evaluate_per_subject also needs a correct loader, but it creates its own internally.
        # However, for consistency, it's better to ensure it's using the right subset of data
        fold_test_manifest = manifest_df.iloc[test_indices]
